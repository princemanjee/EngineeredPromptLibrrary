# Any Programming Language to Python Converter — Context Engineering Template v3.0

> Upgraded from: `PromptLibrary-2.0/Markdown/any_programming_language_to_python_converter.md`
> Strategy: Chain-of-Thought (primary) + Self-Refine (Pythonicity loop)
> v3.0 enhancements: expanded persona with anti-traits, SELF_REFINE block, QUALITY_DIMENSIONS
> table, 5-phase instructions, domain-adaptive context signals, full Observe→Analyze→Draft→
> Critique→Revise→Conclude CoT pattern, complexity-scaled constraints, process-inclusive
> RESPONSE_FORMAT, language-specific FLEXIBILITY branches, and comprehensive METRICS table.
...
**Why this works**: Satisfies Construct Mapping Depth (Logic Mapping annotates Java→Python for every construct), Pythonic Quality (getter replaced with `@property`), Documentation Completeness (`__repr__` added, docstrings present), PEP 8 Compliance (snake_case throughout), and Process Integrity (`[CRITIQUE FINDINGS:]` and `[REVISIONS APPLIED:]` blocks both present and addressed before final code).
...
**Why this works**: Demonstrates the most common C-family anti-pattern and its fix. The critique is specific (names the exact construct), the rationale is explicit, and both `[CRITIQUE FINDINGS:]` and `[REVISIONS APPLIED:]` are visible, satisfying Process Integrity at 100%.
...
**Why it's wrong**: The wrong output violates Pythonic Quality (callback pyramid is not idiomatic Python), Construct Mapping Depth (no mapping annotation for the async pattern), and Process Integrity (the critique phase was skipped — no `[CRITIQUE FINDINGS:]` block exists). It also produces code that a Python developer would immediately reject in code review.
