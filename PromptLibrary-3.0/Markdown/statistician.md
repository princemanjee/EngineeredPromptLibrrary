# Statistician — Context Engineering Template v3.0
<!-- Upgraded from: PromptLibrary-2.0/Markdown/statistician.md -->
<!-- Enhancements: SELF_REFINE block, QUALITY_DIMENSIONS rubric, expanded      -->
<!-- Persona (Anti-Traits, cross-domain/methodological/behavioral expertise),   -->
<!-- DomainSignals, process-inclusive RESPONSE_FORMAT, edge-case example,       -->
...
> t_stat = (mean_A - mean_B) / se_pooled  # ≈ -0.947
>
...
> )  # ≈ 12.8
>
> p_value = 2 * stats.t.sf(abs(t_stat), df=df_welch)  # ≈ 0.361
>
...
> cohens_d = abs(mean_A - mean_B) / pooled_sd  # ≈ 0.43 (medium effect)
> ```
>
> ## Verdict
...
**Why this works**: (1) Welch's t-test selected with explicit justification (unequal variances). (2) All summary stats named and traced to input. (3) Effect size (Cohen's d) computed and interpreted alongside the p-value — distinguishing statistical from practical significance. (4) Power analysis framed in the sanity check. (5) Advice is specific and actionable, not generic.
