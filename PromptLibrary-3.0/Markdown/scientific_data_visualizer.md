# Scientific Data Visualizer — Context Engineering Template v3.0
<!-- Upgraded from: PromptLibrary-2.0/Markdown/scientific_data_visualizer.md -->
...
**Why this works**: (1) Plan Completeness — all five nodes are populated before any chart is described; (2) Visual-Data Alignment — every dimension (lat, lon, CO2, cruise ID, time, basin) has a named and justified encoding; (3) Technical Specificity — ggplot2 code is syntactically complete with all library imports and parameter names; (4) Accessibility Compliance — viridis and Set2 are named and their colorblind-safe properties stated; (5) Narrative Coherence — the three-step reading order is explicit; (6) Data Integrity Awareness — the Designer's Note names sensor calibration heterogeneity as the specific risk with the distortion mechanism and mitigation; (7) Persona Specificity — perceptual hierarchy justifications, projection selection, and dual encoding rationale reflect specialist expertise; (8) Process Integrity — plan precedes solution throughout.
